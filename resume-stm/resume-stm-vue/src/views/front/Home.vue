<template>
  <div style="min-height: 1000px; background-image: linear-gradient(135deg, #e3f2fd 0%, #e8f5e9 100%);">
    <img src="@/assets/imgs/banner.jpg" alt="" style="width: 100%; ">
    <div style="margin: 25px auto; width: 70%; text-align: center">
      <el-input size="large" v-model="data.name" placeholder="请输入你感兴趣的职位" style="width: 800px; margin-right: 5px"></el-input>
    ...
    <div style="margin: 0 auto; width: 70%; text-align: center">
      ...
      <div style="display: flex">
        <div @click="navTo('/front/positionDetail?id=' + data.leftDownAd.positionId)" style="cursor: pointer; flex: 1"><img style="width: 100%; height: 240px; border-bottom-left-radius: 10px" :src="data.leftDownAd.img" alt=""></div>
        <div @click="navTo('/front/positionDetail?id=' + data.centerDownAd.positionId)" style="cursor: pointer; flex: 2; margin: 0 2px"><img style="width: 100%; height: 240px" :src="data.centerDownAd.img" alt=""></div>
        <div @click="navTo('/front/positionDetail?id=' + data.rightDownAd.positionId)" style="cursor: pointer; flex: 1"><img style="width: 100%; height: 240px; border-bottom-right-radius: 10px" :src="data.rightDownAd.img" alt=""></div>
      </div>
      <div style="margin: 30px; font-size: 30px; font-weight: bold; text-align: center">热招职位</div>
      <div>
            ...
            <el-row :gutter="10">
              <el-col :span="8" v-for="it in data.positionData" style="margin-bottom: 20px">
                <div class="card" style="cursor: pointer" @click="navTo('/front/positionDetail?id=' + it.id)">
                  <div style="display: flex; padding: 10px; background-color: #f8f9fa; border-radius: 8px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1)">
                    <div style="flex: 1; text-align: left; font-size: 16px; font-weight: bold; color: #333333">{{ it.name }}</div>
                    <div style="width: 100px; text-align: right; color: #e6a23c; font-size: 16px; font-weight: bold">{{ it.salary }}</div>
                  </div>
                  <div style="padding: 10px; text-align: left">
                    <el-tag style="margin-right: 5px; margin-bottom: 5px" type="info" v-for="tag in it.tagList">{{ tag }}</el-tag>
                  </div>
                  <div style="display: flex; align-items: center; padding: 10px; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1)">
                    <div style="width: 35px">
                      <img :src="it.employAvatar" alt="" style="width: 35px; height: 35px; border-radius: 50%; border: 1px solid #cccccc">
                    </div>
                    <div style="width: 80px; font-size: 14px; color: #666666">{{ it.employName }}</div>
                    <div style="flex: 1; font-size: 14px; color: #999999">{{ item.name }}</div>
                    <div style="width: 80px; font-size: 14px; color: #666666">{{ it.employStage }}</div>
                  </div>
                </div>
              </el-col>
            </el-row>
